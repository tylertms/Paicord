...
private let logger = DiscordGlobalConfiguration.makeLogger(
  "DBM.HTTPRateLimiter"
)
...
actor HTTPRateLimiter {
  ...
  private struct Bucket: Hashable, CustomStringConvertible {
    ...
    var description: String {
      "Bucket(" + "bucket: \(bucket.debugDescription), " + "limit: \(limit), "
        + "remaining: \(remaining), "
  ...
  private func globalRateLimitAllows() -> Bool {
    ...
    if self.requestsThisSecond.id == globalId {
      if self.requestsThisSecond.count
        >= DiscordGlobalConfiguration.globalRateLimit
      {
        logger.warning(
