...
public struct DiscordLocaleDict<C: Codable>: Codable,
...
public struct DiscordTimestamp: Codable, Hashable {
  ...
  public func encode(to encoder: any Encoder) throws {
    ...
    let str =
      "\(f.year)-\(f.month)-\(f.day)T\(f.hour):\(f.minute):\(f.second).\(f.mili)+00:00"
...
extension DiscordTimestamp: Equatable {
  public static func == (lhs: DiscordTimestamp, rhs: DiscordTimestamp) -> Bool {
    return lhs.date == rhs.date
  }
}

extension DiscordTimestamp: Comparable {
  public static func < (lhs: DiscordTimestamp, rhs: DiscordTimestamp) -> Bool {
    return lhs.date < rhs.date
  }
}
...
public struct DiscordColor: Sendable, Codable, Equatable, Hashable,
