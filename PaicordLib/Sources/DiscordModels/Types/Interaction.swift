...
public struct Interaction: Sendable, Codable {
  ...
  public enum Error: Swift.Error, CustomStringConvertible {
    ...
    case optionNotFoundInOption(
      name: String,
      parentOption: ApplicationCommand.Option
    )
    case optionNotFoundInOptions(
      name: String,
      options: [ApplicationCommand.Option]?
    )

    case componentNotFoundInComponents(
      customId: String,
      components: [ActionRow.Component]
    )
    case componentNotFoundInActionRow(customId: String, actionRow: ActionRow)
    case componentNotFoundInActionRows(
      customId: String,
    ...
    public var description: String {
      ...
      case .optionNotFoundInCommand(let name, let command):
        return
          "Interaction.Error.optionNotFoundInCommand(name: \(name), command: \(command))"
      case .optionNotFoundInOption(let name, let parentOption):
        return
          "Interaction.Error.optionNotFoundInOption(name: \(name), parentOption: \(parentOption))"
      ...
      case .componentNotFoundInActionRow(let customId, let actionRow):
        return
          "Interaction.Error.componentNotFoundInActionRow(customId: \(customId), actionRow: \(actionRow))"
      ...
      case .componentWasNotOfKind(let kind, let component):
        return
          "Interaction.Error.componentWasNotOfKind(kind: \(kind), component: \(component))"
      case .dataWasNotOfKind(let kind, let data):
        return
          "Interaction.Error.dataWasNotOfKind(kind: \(kind), data: \(data))"
      }
    }
  }
  ...
  public init(from decoder: any Decoder) throws {
    ...
    self.guild = try container.decodeIfPresent(
      PartialGuild.self,
      forKey: .guild
    )
    ...
    self.guild_locale = try container.decodeIfPresent(
      DiscordLocale.self,
      forKey: .guild_locale
    )
...
extension [Interaction.ApplicationCommand.Option] {
  /// Returns the option with the `name`, or `nil`.
  @inlinable
  public func option(named name: String) -> Interaction.ApplicationCommand
    .Option?
  {
  ...
  @inlinable
  public func requireOption(named name: String) throws
...
extension Interaction {
    ...
    Equatable, Hashable, ValidatablePayload
  {
      ...
      ValidatablePayload
    {
      ...
      public func requireStringSelect() throws -> StringSelectMenu {
        ...
        case .stringSelect(let value):
          return value
        default:
          throw Error.componentWasNotOfKind(
            kind: "stringSelect",
            component: self
          )
        }
    ...
    public enum CodingError: Swift.Error, CustomStringConvertible {
      ...
      public var description: String {
        switch self {
        case .unexpectedComponentKind(let kind):
          return
            "Interaction.ActionRow.CodingError.unexpectedComponentKind(\(kind))"
        case .actionRowIsSupposedToOnlyAppearAtTopLevel:
          return
            "Interaction.ActionRow.CodingError.actionRowIsSupposedToOnlyAppearAtTopLevel"
        }
      }
    }
    ...
    public func requireComponent(customId: String) throws -> Component {
      if let component = self.components.first(where: {
        $0.customId == customId
      }) {
        return component
      } else {
        throw Error.componentNotFoundInActionRow(
          customId: customId,
          actionRow: self
        )
      }
...
extension [Interaction.ActionRow] {
  ...
  /// Returns the component with the `customId`, or throws `Interaction.Error`.
  @inlinable
  public func requireComponent(customId: String) throws
    -> Interaction.ActionRow.Component
  {
    if let component = self.flatMap(\.components).first(where: {
      $0.customId == customId
    }) {
      return component
    } else {
